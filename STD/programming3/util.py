...
def plot(x_values, y_values, title, xlabel, ylabel, filename, labels=None):
    """Plots a line graph
    ...
    Args:
        ...
        labels(list of str): You need to provide labels if you want to plot multiple
                      lines on the same figure. x_values and y_values need to
                      be a nested list, and satisfy
    ...
    if labels is not None:
        for x_value, y_value, label in zip(x_values, y_values, labels):
            ax.plot(x_value, y_value, linewidth=3, marker='o', markersize=10, label=label)
        ax.legend(fontsize=40)
    else:
        ax.plot(x_values, y_values, linewidth=3, marker='o', markersize=10)
